...
export default function Produits() {
        ...
        const fetchQuantities = async () => {
            const results = await Promise.all(
                products.map((product) =>
                    fetch(
                        `${process.env.NEXT_PUBLIC_API_URL}/items_sold_amount_sold/${product.productid}`
                    )
                        .then((res) => res.json())
                        .then((data) => ({
                        ...
                        .catch((err) => {
                            console.error(
                                "Erreur lors du chargement des quantités :",
                                err
                            );
                            return { productid: product.productid, items: [] };
                        })
                )
            );
            setQuantities(results);
        };
    ...
    useEffect(() => {
        ...
        quantities.forEach(({ productid, items }) => {
            let sum = 0;
            let pid = 0;

            items.forEach((item) => {
                const dateStr = item.earning?.created_at;
    ...
    const handleSortChange = (e) => {
        ...
        if (value === "asc") {
            sortedData.sort(
                (a, b) => quantity[a.productid] - quantity[b.productid]
            );
        } else if (value === "desc") {
            sortedData.sort(
                (a, b) => quantity[b.productid] - quantity[a.productid]
            );
        }
                            ...
                            {currentProducts.length > 0 ? (
                                currentProducts.map((product, index) => (
                                    <tr key={index} className="border-t">
                                        ...
                                        <td className="py-3 px-4">
                                            {parseFloat(product.cost).toFixed(
                                                2
                                            )}
                                            €
                                        </td>
